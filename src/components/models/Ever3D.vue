...
<script lang="ts">
import { defineComponent, ref, Ref, reactive, onMounted, toRefs } from "vue";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const updateData = (params: ProductInfoTypes) => {
    ...
    const showImage = (isShow: boolean) => {
      isShowImage.value = isShow;
    }
    ...
    const getStpRender = async(filePath: string) => {
      try {
        ...
        if (success) {
          ...
          fileUrl.value = BASE_IMAGE_URL + fileE3dx;
          startRender();
        } else {
