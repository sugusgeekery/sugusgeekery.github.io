...
import { INTERFACE_ERROR_TEXT, VERIFICATION_CODE_NUMBER, VITE_APP_MODE } from "@/config";
...
export default {
  ...
  async [RootActionTypes.PhoneLogin](store: Store, parameter: { phoneNo: string; }) {
    try {
      const { dispatch, commit } = store;
      // PhoneLogin(parameter).then((res: any) => {
  ...
  async [RootActionTypes.GetAccountInfo](store: Store) {
    try {
      ...
      if (success) {
        ...
        if (VITE_APP_MODE === "production" && !wxOpenId) {
          ElMessage.error("请先关注公众号，已关注过公众号，请取消再重新关注即可登录!");
          return;
        }
        if (!isFirstExist) {
          router.push("/home");
