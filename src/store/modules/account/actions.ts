...
export default {
  ...
  async [ActionTypes.SaveCompanyInfo](store: Store) {
    try {
      ...
      if (!companyPhoneNo) {
        ElMessage.error("请输入公司电话！");
        return;
      }
      if (!provinceId || !cityId || !districtId || !address) {
        ElMessage.error("请选择公司地址！");
        return;
      }
      if (!description) {
        ElMessage.error("请输入公司详情！");
        return;
      }
  ...
  async [ActionTypes.SavePersonQualifyInfo](store: Store) {
    try {
      ...
      if (!idcardBackImgId) {
        ElMessage.error("请上传身份证背面照！");
        return;
      }
      if (!labelIdList.length) {
        ElMessage.error("请选择你拥有的生产能力标签！");
        return;
      }
