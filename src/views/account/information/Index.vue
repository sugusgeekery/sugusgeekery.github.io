              ...
              <div class="form-item">
                ...
                <div class="form-item-picker">
                  <el-date-picker
                    class="form-item-picker-date"
                    prefix-icon=""
                    v-model="companyInfo.publishTime"
                    type="date"
                    placeholder="选择日期"
                    @input="updateDate"
                    @keyup.enter="submitForm()"
...
<script lang="ts">
import { defineComponent, ref, Ref, computed, watch, onUnmounted, onMounted, reactive } from "vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const provinceCityDistrict: Array<ProvinceCityDistrict> | any = computed(() => store.state.account.provinceCityDistrict);
    const userInfo: UserInfo | any = computed(() => store.state.account.userInfo);
    const companyInfo: CompanyInfo | any = computed(() => store.state.account.companyInfo);

    const getInformationDefInfo: Function = (params: any) => store.dispatch(`account/${ActionTypes.GetInformationDefInfo}`, params);
    const updateDefLoginInfo: Function = (params: any) => store.dispatch(`account/${ActionTypes.UpdateDefLoginInfo}`, params);
    const uploadForm: Function = (params: any) => store.dispatch(`account/${ActionTypes.UploadForm}`, params);
    const changeSupplierType: Function = (params: any) => store.dispatch(`account/${ActionTypes.ChangeSupplierType}`, params);
    const saveUserInfo: Function = (params: any) => store.dispatch(`account/${ActionTypes.SaveUserInfo}`, params);
    const saveCompanyInfo: Function = (params: any) => store.dispatch(`account/${ActionTypes.SaveCompanyInfo}`, params);

    const updateInitInfo: Function = (params: any) => store.commit(`account/${MutationTypes.UpdateInitInfo}`, params);
    const updateUserInfo: Function = (params: any) => store.commit(`account/${MutationTypes.UpdateUserInfo}`, params);
    const updateCompanyInfo: Function = (params: any) => store.commit(`account/${MutationTypes.UpdateCompanyInfo}`, params);

    const biu = ref(BASE_IMAGE_URL);
    const regionDataTemp = reactive(regionData);
    ...
    const uploadFile = (e: any) => {
      ...
      while (len > 0) {
        uploadForm({ file: files[files.length - len] }).then((res: any) => {
          ...
          updateUserInfo({ headImgUrl: fileThumPath });
          e.target.value = null;
        });
        len--;
      }
    ...
    const changeUserInfoInput = (value: string, key: string) => {
      const obj = {
    ...
    const changeCompanyInfoInput = (value: string, key: string) => {
      const obj = {
    ...
    const updateProvinceCityDistrict = (value: any) => {
