...
<script lang="ts">
import { defineComponent, ref, Ref, computed, watch, onUnmounted } from "vue";
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const biddingDetail: BiddingDetailTypes | any = computed(() => store.state.bidding.biddingDetail);

    const getTechnology: Function = (params: any) => store.dispatch(`bidding/${ActionTypes.GetTechnology}`, params);
    const getMaterialAndColor: Function = (params: any) => store.dispatch(`bidding/${ActionTypes.GetMaterialAndColor}`, params);
    const getArrangementScheme: Function = (params: any) => store.dispatch(`bidding/${ActionTypes.GetArrangementScheme}`, params);
    const getProductInfo: Function = (params: any) => store.dispatch(`bidding/${ActionTypes.GetProductInfo}`, params);
    const joinBidding: Function = (params: any) => store.dispatch(`bidding/${ActionTypes.JoinBidding}`, params);
    const removeBidding: Function = (params: any) => store.dispatch(`bidding/${ActionTypes.RemoveBidding}`, params);

    const updateBiddingDetail: Function = (params: any) => store.commit(`bidding/${MutationTypes.UpdateBiddingDetail}`, params);

    const onContextmenu = (event: any, productInfoIndex = -1) => {
      const { productInfos = [], biddingIndex = 0 } = biddingDetail || {};
      if (biddingIndex < 2) {
        return;
      }
